...
interface BasicInfoStepProps {
  initialData: Partial<SocialAtomFormData>
...
export function BasicInfoStep({ initialData, onNext }: BasicInfoStepProps) {
  const [localData, setLocalData] = useState<Partial<SocialAtomFormData>>({
    ...
    socialAtomName: initialData.socialAtomName || '',
    socialAtomDescription: initialData.socialAtomDescription || '',
    hasImage: initialData.hasImage || false,
    hasIdentity: initialData.hasIdentity || false,
  })

  // Set initial values based on platform and username if not already set
  useEffect(() => {
  ...
  const handleNext = () => {
    ...
    // Pass data to parent and trigger navigation
    onNext(localData)
  }
